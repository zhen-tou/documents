...
- **浅层 $l_1\text{–}l_2$**：几乎就是边缘/纹理图，对**去雨条纹、去模糊、去雪**最敏感（这类退化正是"结构级"破坏）
- **中层 $l_5\text{–}l_7$**：对**低光增强、超分、去雾**最敏感（依赖局部对比度和中层语义）
- **深层 $l_8\text{–}l_{10}$**：编码全局语义，对**噪声**最敏感（因为噪声破坏的是语义可读性，浅层反而"被骗过"）
...
$$
s_l \;=\; \tfrac{1}{2}\bigl(s_l^{\cos} + s_l^{\text{dist}}\bigr)
$$

其中：

- **余弦相似度分量**（衡量方向一致性，即语义/结构对齐）：
  $$
  s_l^{\cos} \;=\; \frac{1}{N}\sum_{n=1}^{N}\frac{\langle U_l^{lq}[n],\; U_l^{hq}[n]\rangle}{\|U_l^{lq}[n]\|_2\;\|U_l^{hq}[n]\|_2}
  $$
- **归一化 $L_2$ 距离相似度分量**（衡量幅值差距，把距离映射回 $[0,1]$ 的相似度）：
  $$
...
| 权重 | 公式 | 温度作用 | 用途 |
| --- | --- | --- | --- |
| **$q_l$（条件权重）** | $q_l = \dfrac{\exp(s_l)}{\sum_{k\in\mathcal{L}}\exp(s_k)}$ | 相似度越高，权重越大 | **fuse 出条件** $U_{fuse}=\sum_l p_l U_l$，注入 DiT 做 cross-attn 的 K/V |
| **$r_l$（监督权重）** | $r_l = \dfrac{\exp(1-s_l)}{\sum_{k\in\mathcal{L}}\exp(1-s_k)}$ | 相似度越低，权重越大 | 给不可靠层加更大 loss：$\mathcal{L}_{feat}=\sum_l r_l\,\ell^{feat}_l$ |

$q_l$ 与 $r_l$ 的和取 softmax 前**互为反号**，构成"**你会的我拿来用；你不会的我使劲练你**"的分工。作者的原话是：*"$q_l$ selects reliable content features for conditioning, while $r_l$ focuses supervision on layers that need stronger restoration."*
...
$$
p_l = \text{softmax}\bigl(\rho_\psi([y_{lq}, U_l])\bigr),\quad l\in\mathcal{L}
$$
